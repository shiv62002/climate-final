const countryCode = new URLSearchParams(window.location.search).get("code") || "USA";
    ...
    const files = {
      ...
      disaster: "data/economic-damages-from-natural-disasters-as-a-share-of-gdp.csv"
    };
    ...
    let datasets = {}, yearRange = [2000, 2022];
    ...
    function drawChart(animate = false) {
      ...
      const xLabel = variableLabels[xKey], yLabel = variableLabels[yKey];
      ...
      g.append("text").attr("x", w / 2).attr("y", h + 35).attr("text-anchor", "middle").text("Year");
      g.append("text").attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -45).attr("text-anchor", "middle").text(`${xLabel} (Normalized)`);
      g.append("text").attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", w + 45).attr("text-anchor", "middle").text(`${yLabel} (Normalized)`);

      const line1 = d3.line()
        .curve(d3.curveMonotoneX)      // ← ensures x is always increasing
        .x(d => x(d.year))
        .y(d => y1(d.x));
      ...
      g.selectAll(".dot1").data(merged).enter().append("circle")
        .attr("r", 3).attr("fill", "#1c3d5a")
        .attr("cx", d => x(d.year)).attr("cy", d => y1(d.x))
      ...
      g.selectAll(".dot2").data(merged).enter().append("circle")
        .attr("r", 3).attr("fill", "#e66101")
        .attr("cx", d => x(d.year)).attr("cy", d => y2(d.y))
      ...
      legend.append("div").html(`<span class="legend-swatch" style="background:#1c3d5a;"></span>${xLabel}`);
      legend.append("div").html(`<span class="legend-swatch" style="background:#e66101;"></span>${yLabel}`);
    }
    ...
    function averageTemperature(data) {
      ...
      return Array.from(grouped, ([year, value]) => ({ year, value })).filter(d => d.year >= 2000 && isFinite(d.value));
    }
    ...
    Promise.all([
      d3.json("data/countryNameToCode.json"),  // add this
      d3.csv(files.temperature),
      d3.csv(files.wheat),
      fetch(files.gdp).then(r => r.text()).then(t => d3.csvParse(t.split("\n").slice(4).join("\n"))),
      fetch(files.food).then(r => r.text()).then(t => d3.csvParse(t.split("\n").slice(4).join("\n"))),
      ...
      datasets.gdp = gdpRows.filter(d => d["Country Code"] === countryCode).flatMap((d) => {
        const vals = [];
        for (let y = 2001; y <= 2022; y++) {
          const prev = +d[y - 1], curr = +d[y];
      ...
      datasets.food = foodRows.filter(d => d["Country Code"] === countryCode)
        .flatMap(d => Object.keys(d).filter(k => k.match(/^\d{4}$/)).map(y => ({ year: +y, value: +d[y] })))
        .filter(d => d.year >= 2000 && isFinite(d.value));

      const yearKey = Object.keys(disasterRows[0]).find(k => k.toLowerCase() === "year");
      const codeKey = Object.keys(disasterRows[0]).find(k => k.toLowerCase().includes("code"));
      const valueKey = Object.keys(disasterRows[0]).find(k => k.toLowerCase().includes("damage") && k.toLowerCase().includes("gdp"));

      if (!yearKey || !codeKey || !valueKey) {
        console.error("Missing expected columns. Found keys:", Object.keys(disasterRows[0]));
      } else {
